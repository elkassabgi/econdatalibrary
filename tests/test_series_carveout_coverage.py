...
def test_carveout_like_prefixes_cover_both_id_shapes():
    ...
    `worldbank_wdi:FP.CPI.TOTL.ZG` and other two-part carved ids have no third
    segment, so their SQL exclusion matched 0 rows for as long as it existed. The JS gate still
    covered them, so this was defence-in-depth rather than an open door — but carved series have
