...
minutes — the log says so. It still could not stop anything:
    ...
    10:02  the source starts and submits all 150 slugs to a 6-worker pool
...
The source printed nothing for 150 minutes and no timeout message ever appeared, because the exception
...
def test_detect_change_runs_inside_the_unit_deadline():
    """The second outage (run 31224822131, WITH the pool fix on board). A source entered at 23:23
    ...
    assert "_unit_deadline(" in head.rsplit("try:", 1)[-1] or            "_unit_deadline(" in head[-600:], (
        ...
        "eats the entire run again (212 minutes of one source, twice)")
