...
> each unit declares its own output path(s). `central_banks` → units `boc`,`snb`,`riksbank`,… each
...
- **S2 `extend_by_date`** (~18–22) — true `since=last_obs` delta within existing series. Read
  ...
  merge, advance cursor. Covers tcmb, treasury, ofr, bcb, bcrp, fred_releases (also fixes the
  0-obs-marked-done bug), eia, fed_board, defillama, riksbank, ecb, who_gho, the 8 partials.
- **S3 `sdmx_delta`** (~25–30) — SDMX specialization of S2: `?updatedAfter=<last_success>` to detect,
  ...
  insee_bdm/melodi, and the PxWeb/SIDRA NSOs (dst, scb, ssb, statfin, ine_spain, stat_*, hagstofa, cso,
  ibge, ipea, idb, gus_bdl).
...
**In-flight protection:** cbs_nl, gus_dbw, dbnomics-ISTAT are seeded `status=running, owner=firstpass`;
...
mark done" class (bea, fred_releases) and the "skip series if key present" freeze (67 sources).
...
- **404 / non-trading-day:** definitive-empty for that key/date, recorded in a skip-set (fixes tcmb
...
- **Atomicity:** every write `.tmp`+`os.replace` (local) / staged-then-PUT (R2); state written *after*
  ...
  freedomhouse, tcmb checkpoint, bcb/bcrp).
...
2. **Shared helpers:** `merge_and_write()` (atomic, dedup, never-shrink), Transient/Definitive contract,
   rate governor. Unit-test against bcb/tcmb fixtures.
3. **S2 first** (highest currency, lowest risk): tcmb, treasury, ofr, bcb, bcrp, fred_releases (+fix
