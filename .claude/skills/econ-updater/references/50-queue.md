...
| stat_slovenia 1/2 "returned 200 but parsed 0 rows" (08-04) | Re-swept in the same run: healthy rotating sweep (74/146 groups, offset saved); note now "4/21 parsed 0 rows" — its fetcher does NOT name the failing sub-units in the log (hagstofa/cso do). NEXT: add the naming, then classify the 4 (suspected hagstofa-style archival class) |
