...
| census: cursors were per-series store keys ('eits/advm3\|dim=val\|…') vs a table-grain catalog ('census:eits__advm3', split parts '#part') — 12,019/12,019 unmapped | FIXED cb051654 (cycle 23): table-grain cursors keyed on the merge's own max date + a general split-part expansion rule in the mapper (exact miss → catalog `<id>#%` rows; wildcards escaped). Proof run 31058738621 queued. Follow-up CLOSED 2026-08-06: the 22 legacy series-grain ids (marts/mrts/vip/resconst/ressales/advm3/m3) re-derived byte-exact via core.derive_csv and uploaded 22/22 — they refresh whenever their eits tables change (the table id maps; per-series freshness rides the same store) |
...
| cso: up to 222 matrices "no subject mapping" retried forever, ~45% of every budget (verdict 08-03) | FIXED a8d36d7f (cycle 24): SIH13/SIA208 probed live — they EXIST with full metadata but are absent from the Search listing (R61); still-unroutable matrices now route via their OWN ReadMetadata extension.subject. Proof run queues after census |
...
| worldbank_wdi 10,255 unmapped every run | FIXED 9399bfe0 (cycle 26): cursors fold to indicator grain ('WDI:<ind>:<geo>' store vs 'worldbank_wdi:<ind>' catalog). Proof after census/cso |
| fhfa bare-key cursors (07-30 verdict) | MEASURED stale: the cube-prefix fix (2c371006, 08-02) postdates the last attempt — self-clears on the monthly rotation |
