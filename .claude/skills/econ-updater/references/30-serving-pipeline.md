...
1. **Catalogue the series** (pick ONE path):
   ...
   - Proof: `SELECT COUNT(*) FROM series WHERE source_id='<sid>'` > 0. **CORRECTED 2026-09-07 (R875 #2): `ix_series_source_id` EXISTS on the live catalog.db and this query is an index SEARCH, not a full scan.** Measured on the 11.9 GB file — `PRAGMA index_list('series')` lists `ix_series_source_id(source_id)`; `EXPLAIN QUERY PLAN` says `SEARCH series USING INDEX ix_series_source_id (source_id=?)`; 50 runs on `yale_epi`, the last source in key order, take 0.0027 s. `core/catalog.py:39` creates it with `IF NOT EXISTS`, so any run of the schema DDL builds it. The old warning here — and the "several-minute index build" still listed as an open decision in the 2026-09-05 handoff — described a state that no longer holds. **The rule that survives: run `EXPLAIN QUERY PLAN` before asserting a predicate scans.** Citing R721 by id instead of running it is exactly how the false premise got written into three documents and a source comment.
...
5. **Health-gate red** — step "Health gate (fail past 2x SLA)" runs `python -m updater.health --fail-past-2x-sla` (updater-daily.yml:325-328). Classes, worst-first: `RED-SLA` (job hasn't succeeded within 2× cadence), `RED-DATA` (job "succeeds" but newest observation is stale), `RED-UNRUN` (adapter built, never succeeded), `ATTENTION` (partial/failed/running), `PENDING` (live, no adapter) (updater/health.py:24-28, 235-248, 330, 340-343). Interactive views: `python -m updater.health` (table + health.json), `--red`, `--json` (updater/health.py:11-13). Before "fixing" anything, apply the standing misreads (tools/gen_runbook.py:286-307, 447-455):
   ...
   - `obs_count` swings by 168M: it is the STORE TOTAL for most fetchers, not rows this run - 3 of ~123 finalize call sites pass a real added count (R326, mechanism corrected 2026-09-03) — count the store instead.
