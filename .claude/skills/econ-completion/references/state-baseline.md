...
- **W6 honesty**: /v1/stats publication (RESERVED); R-client claim (no econ R client); ~~sdmx_nso licence drift~~ — **premise withdrawn 2026-08-30**: `sdmx_nso` has **0 series rows locally and does not exist in live D1 at all** (0 series, 0 `source_counts`, 0 `source` rows), so there are not two records to reconcile. It is one stale local `source` row naming ISTAT, which is separately live as `istat`. Local debris, not a licence question.
