...
- `/v1/stats` serves **3,190,863,550 series / 24,054,980,219 observations**, `as_of` 2026-08-23, with `recalculating: true` and a note saying the totals may change. MEASURED at the live endpoint 2026-09-16 (`curl https://econdl-api.elkassabgi.workers.dev/v1/stats`), which is the only thing that can settle it. It does NOT serve the July census (79.8B / 7.73B) — that figure is WITHDRAWN and must not be requoted — and it does not serve the 33.9B / 3.90B figure this line used to call the honest store either. Publishing a revised census figure is still RESERVED to Ahmed; `tools/series_census.py --publish` has a ±20% gate needing `--force-publish`. This line was twice asserted from a document rather than the system, which is failure class B in failure-classes.md.
...
- **W1 key collisions** (re-key = RESERVED): eia (52.9% of ids; 566 MB single "series"), idb, unctad ×2 (Flow; publisher-confirmed), damodaran (721 wrong values; serves India's tax rate as its default spread), bea/defillama/istat minor. Five giants unswept: statcan, eurostat, cbs_nl, oecd, ilostat.
...
- **W6 honesty**: /v1/stats publication (RESERVED); R-client claim (no econ R client).
