...
| E. Unanchored matching | Substring/prefix matches that collide (R129: `series/imf_<redacted>` matches `imf_fsire`; R112: `ppi` inside "shipping"). | Anchor on a delimiter you control (`series/imf_<redacted>%3A`); match structured data, never formatted sentences; assert `len(keys)==len(set(...))` before name-keyed comparisons. |
