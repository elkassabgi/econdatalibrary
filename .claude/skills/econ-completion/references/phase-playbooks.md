...
Key tasks: fix `ledger_check.py --digest`; fix stale doc counts; capture baselines (audit, untouched files, retry-queue provenance incl. the `abs`/`ilostat` GROUP BY, the ONE `SELECT_SOURCES` rows_read measurement).
