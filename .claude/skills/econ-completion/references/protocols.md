...
4. The list: deleting non-re-crawlable data · un-gating a DISPUTED licence · auth & billing · sending email as Ahmed · ANY change that alters PUBLIC series ids (incl. every key-collision re-key in Phase 4) · `/v1/stats` publication · health-gate policy for bounded broken minorities · serving cross-sectional data (`oecd`, `gleif`) · `norgesbank` un-gating provenance · `worldbank_pink` 26 rows · `sec_edgar`/`sec_edgar_xbrl` id crossing.