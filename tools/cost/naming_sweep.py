...
def main() -> int:
    ...
    print()
    print("THE NUMBER TO MOVE IS NOT THIS ONE. tests/test_failure_labels_ratchet.py counts")
    print("only IN-LOOP bare calls, because on a single-unit fetcher \"1/1 failed\" already")
    print("names the unit — this tool's total is a deliberate superset of that. The ratchet")
    print("fails if its count rises and tells you what to lower BUDGET to when it falls.")
    print()
    print("A label on a single-unit fetcher is still worth adding: the identity is known but")
    print("the REASON is not, and the raise site usually has it — a status, a byte count, an")
    print("exception type. That is why penn_world_table, pwt, gleif and _iep were labelled on")
    print("2026-09-03 even though their calls are not in-loop.")
    print()
    print("Sources whose failures were CURRENT were done first: the sixteen the cloud health")
