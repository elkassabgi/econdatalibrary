...
def sample(sid, full=False):
    """(series_key, obs_date) pairs sampled from one source, cheaply."""
    ...
    for f in files:
        try:
            ...
            for i in range(pf.num_row_groups):
                ...
                rows += list(zip(t[kcol].to_pylist(), t["obs_date"].to_pylist()))
                # --full means FULL. Leaving the row cap active under --full would
                # have made the exhaustive run a 200k-row sample wearing its name,
                # and the whole point of this audit is a library-wide decision.
                if not full and len(rows) >= MAX_ROWS:
                    return rows
...
def classify(rows):
    ...
    for k, ds in per.items():
        ...
        gaps.sort()
        med = gaps[len(gaps) // 2]
        s = ds[len(ds) // 2]                                  # a mid-series sample
