...
# sources, nearly all of them genuine: treasury's US debt outstanding from 1790, vdem from
...
def main() -> int:
    ...
    #
    # MEASURED 2026-09-03 with the candidate resolution above: some registry sources have no
    # local parquet store under ANY candidate (gii and pxweb among them). Note the result moved
    # BECAUSE of this file's own fix -- more read as absent when only clean_full/<source_id> was tried, and
    # sec_edgar + sec_edgar_xbrl came into range once out_dir and the clean_grouped tier were
    # followed. Quote the post-fix result, not the one that motivated the change.
    if hits:
        return 1
