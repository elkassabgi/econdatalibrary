"""Materialize one CSV per PIP table (COUNTRY x FREQ x INDICATOR, key parts 4-6) to R2.

Contract identical to derive_imts_tables.py / derive_pxweb_flowgrain.py:
    header series_id,obs_date,value · series_id column = the native 7-part store key ·
...
def main() -> int:
    ...
    cur = q.execute(f"""
        SELECT 'PIP:' || split_part(series_key,'.',4) || '.' ||
               split_part(series_key,'.',5) || '.' ||
               split_part(series_key,'.',6)             AS tbl,
