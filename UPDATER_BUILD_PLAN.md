...
- Pre-push scan of the staged tree for the **literal secret VALUES** read out of `.env` (every BLS/BEA/CENSUS/EIA/NOAA/INSEE/USDA/R2_*/HFDL value grepped verbatim against every staged blob) — grepping for variable *names* is not enough: a key hardcoded without its name (exactly the hf `local_backfill` incident) sails past a name scan. Plus a regex for 20+-char hex/base64 blobs as the catch-all, each hit individually justified.
