...
# clean_full/<dir> names the running jobs write to (cbs_nl and gus_dbw; the relay-era
# ISTAT re-pull that once sat here went with the relay). We match on BOTH
...
def _csv_desktop_excluded(source_id: str) -> set:
    ...
    answer 'no served object' forever). eurostat:migr_asyrescra (213,650,346 rows, 404 by the
    NUMBERS.md decision) is the member today. Cached like _csv_grain."""
    cache = _csv_desktop_excluded.__dict__.setdefault("_cache", {})
    if not cache:
        try:
            for e in registry.load().get("sources", []):
                cache[e.get("source_id")] = set(str(x) for x in (e.get("csv_desktop_exclude") or []))
        except Exception:                                   # noqa: BLE001
            return set()
        cache.setdefault("__loaded__", set())
    return cache.get(source_id, set())


_DESKTOP_OWED_TOO_LARGE = "too large for the cloud derive; desktop: core.derive_csv --only, read back, clear"
_DESKTOP_OWED_BUDGET = ("budget-deferred flow-grain id; the store file is absent on later runners (r2), "
...
def _book_csv_desktop_owed(store, source_id: str, large: dict, reason: str = _DESKTOP_OWED_TOO_LARGE) -> None:
    ...
    retry raises ResolveError forever) as a DEBT with a per-id home — csv_desktop_owed — so
    health surfaces them and the desktop derive can pay them. Never csv_retry_queue. Ids the
    registry declares unserved by decision are skipped and named. Loud when it cannot book."""
    if not large:
        return
    excluded = _csv_desktop_excluded(source_id)
    skipped = sorted(s for s in large if s in excluded)
    if skipped:
        print(f"[orchestrator] {source_id}: {len(skipped)} id(s) NOT booked as desktop debt — "
              f"registry `csv_desktop_exclude` says unserved by decision: {skipped[:5]}", flush=True)
        large = {s: n for s, n in large.items() if s not in excluded}
        if not large:
            return
    if store is None:
        print(f"[orchestrator] {source_id}: {len(large):,} flow(s) owed to the desktop derive "
              f"and NO state store to book them in — NOT BOOKED: {sorted(large)[:5]}", flush=True)
        return
    store.note_csv_desktop_owed(source_id, [(sid, n, reason) for sid, n in sorted(large.items())])
    print(f"[orchestrator] {source_id}: {len(large):,} flow(s) booked in csv_desktop_owed "
          f"({reason.split(';')[0]}; e.g. {sorted(large)[:3]}); served CSVs for these stay at "
          f"the previous vintage until the desktop derive pays the debt", flush=True)
...
def _derive_changed_csvs(unit, res, blob, store=None):
    ...
    try:
        ...
        # BUDGET-DEFERRED FLOW-GRAIN IDS GO THE SAME WAY (final-diff review, condition 2). On the
        # r2 backend the parquet a flow was merged into exists only on the runner that wrote it
        # (blob.write_table_atomic keeps a local copy), so a retry on a later runner raises
        # ResolveError every run, attempts climb without bound, and health reads none of it —
        # the served CSV stays at the old vintage with no ATTENTION line. Booking them as a
        # desktop debt makes the staleness visible and payable; nothing is enqueued.
        budget_owed: list = []
        ...
        elif large or budget_owed:
            # Too large for the runner and/or not reached inside the budget, nothing broken: a
            # disclosed, non-demoting note; the durable record is the csv_desktop_owed row
            # health holds at ATTENTION.
            note = (f"csv coverage note: {len(large)} of {len(ids)} flow(s) too large for the "
                    f"cloud derive and {len(budget_owed)} not reached inside the derive budget — "
                    f"owed to the desktop (csv_desktop_owed), none failed")
