#!/usr/bin/env python3
"""Statistics Latvia (CSP) — full PxWeb ingest, 1920–present.

License: Creative Commons Attribution 4.0 (CC BY 4.0)
Source: https://data.stat.gov.lv/
No API key required.

Coverage: Two databases (OSP_OD, OSP_PUB) with all statistical tables:
  * Population and demographics (1920–present)
...
Run: python jobs/ingest_stat_latvia.py
"""
...
OUT  = os.path.join(ROOT, "data", "clean_full", "stat_latvia")
BASE = "https://data.stat.gov.lv/api/v1/en"
UA   = {"User-Agent": "Econ-Fin Data Library admin@hfdatalibrary.com"}
RATE = 0.3
MAX_CELLS = 100_000
DATABASES = ["OSP_PUB", "OSP_OD"]
...
def crawl_catalog() -> list[dict]:
    """BFS crawl across both databases, return list of table dicts."""
    ...
    log("Crawling Statistics Latvia catalog...")
    tables = []

    for db in DATABASES:
        queue = deque([(db, "")])  # (db, path)
        visited = set()

        while queue:
            cur_db, path = queue.popleft()
            key = f"{cur_db}/{path}"
            if key in visited:
                continue
            visited.add(key)

            url = f"{BASE}/{cur_db}/{path}/" if path else f"{BASE}/{cur_db}/"
            data = get_json(url)
            time.sleep(RATE)
            ...
            for item in data:
                ...
                itext = item.get("text", "")
                child_path = f"{path}/{iid}".lstrip("/") if path else iid
                if itype == "t":
                    tables.append({"db": cur_db, "path": child_path, "id": iid, "text": itext})
                elif itype == "l":
                    queue.append((cur_db, child_path))
...
TIME_CODES = (
    "tid", "time", "year", "period", "periods", "datums", "datum",
    "gads", "gadi", "gadu",          # year (lv)
    "menesis", "menesi", "manesis",  # month (lv)
    "kvartals", "kvartalls", "ceturksnis",  # quarter (lv)
    "nedela",                        # week (lv)
)
...
def is_time_dim(code: str, values: list[str]) -> bool:
    ...
    `time: true` flag (Latvia's TIME variable carries it — see query_table /
    parse_jsonstat2). This fallback is used only when that flag is absent.

    A dimension is 'time' only if its code names a time dim, OR its values parse
    via parse_date() to SANE years (~1900..current_year+2). The old value test
    matched ^\\d{4}[MQKHW]?\\d*$, which false-matched non-time PxWeb numeric codes
    (AREA municipality codes like 4601/5001/9610, ContentsCode/INDICATOR numeric
    codes) as years — the origin of the 9999-12-31 / 100-12-31 corruption."""
...
def parse_jsonstat2(data: dict, prefix: str, time_code: str | None = None) -> list[tuple[str, dt.date, float]]:
    ...
    variable flagged `time: true`). When supplied we lock onto exactly that dimension
    instead of guessing, so a non-time numeric category (AREA / ContentsCode /
    INDICATOR) can never be mistaken for the time axis. Falls back to a two-pass
    is_time_dim() search only when the flag is absent."""
    results = []
    try:
        dim_ids   = data.get("id", [])
        ...
        # Time-dim selection. Pass 1 (authoritative): the metadata-flagged time code,
        # else a literally-named time dim. Pass 2 (fallback): value-parsing via
        # is_time_dim. A real/named time dim ALWAYS beats a non-time numeric category.
        if time_code is not None:
            time_dim_idx = next((i for i, did in enumerate(dim_ids) if did == time_code), None)
        if time_dim_idx is None:
            time_dim_idx = next(
                (i for i, did in enumerate(dim_ids)
                 if str(did).strip().lower() in TIME_CODES), None)
        if time_dim_idx is None:
            time_dim_idx = next(
                (i for i, did in enumerate(dim_ids)
                 if is_time_dim(did, dim_codes[i])), None)
...
def query_table(table_info: dict) -> list[tuple[str, dt.date, float]]:
    ...
    # (Latvia's TIME variable carries it). Lock onto it everywhere below.
...
def main():
    ...
    log(f"Processing {len(tables)} Latvia tables")

    # Group by db/first-level subject for per-file checkpointing
    by_group: dict[str, list] = defaultdict(list)
    for t in tables:
        parts = t["path"].split("/")
        grp = f"{t['db']}_{parts[0]}" if parts else t["db"]
    ...
    log(f"DONE: {total_obs:,} total Statistics Latvia observations")
